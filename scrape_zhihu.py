...
def scrape_zhihu_article(url):
    ...
    # 获取文章内容 - 知乎专栏的内容区域
    article_content = soup.find('div', class_='RichText Post-RichText ztext')
    ...
    for img in img_tags:
        ...
        if src:
            ...
            if local_filename:
                ...
                img_url_mapping[src] = local_filename  # 也映射原始路径
    ...
    def process_element(element):
        ...
        for child in element.children:
            if child.name is None:
                ...
                if src_clean in img_url_mapping:
                    result.append('![' + alt + '](/imgs/' + img_url_mapping[src_clean] + ')')
                elif src in img_url_mapping:
                ...
                if fig_img:
                    src = fig_img.get('src') or fig_img.get('data-src', '') or fig_img.get('data-original', '')
                    alt = fig_img.get('alt', '')
                    src_clean = src.split('?')[0] if '?' in src else src
                    if src_clean in img_url_mapping:
                        result.append('![' + alt + '](/imgs/' + img_url_mapping[src_clean] + ')')
...
if __name__ == '__main__':
    # 知乎专栏文章链接列表
    urls = [
        'https://zhuanlan.zhihu.com/p/644982948',
        'https://zhuanlan.zhihu.com/p/645280474',
        'https://zhuanlan.zhihu.com/p/645336875',
        'https://zhuanlan.zhihu.com/p/646220657',
        'https://zhuanlan.zhihu.com/p/647579470',
        'https://zhuanlan.zhihu.com/p/660328880'
    ]
    
    for url in urls:
        try:
            scrape_zhihu_article(url)
            time.sleep(2)  # 间隔2秒，避免请求过快
        except Exception as e:
            print('处理 ' + url + ' 失败: ' + str(e))