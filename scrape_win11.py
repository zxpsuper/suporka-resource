...
def download_and_convert_image(url, save_dir):
    try:
        headers = {
        ...
        if not filename:
            # 如果URL中没有文件名，生成一个
            filename = hashlib.md5(url.encode()).hexdigest()[:8] + '.webp'
        else:
            # 替换扩展名为webp
        ...
        try:
            ...
            print('图片转换失败，尝试直接保存: ' + str(e))
            # 如果转换失败，直接保存原始文件
            with open(filepath, 'wb') as f:
                f.write(response.content)
        
        print('下载并转换图片成功: ' + filepath)
...
def scrape_article(url):
    ...
    for img in img_tags:
        ...
        if src:
            ...
            local_filename = download_and_convert_image(full_url, imgs_dir)
            if local_filename:
